...
Color trace_ray(Ray *original_ray, int level)
{
  ...
  */
  ...
  //kolla s� detta verkligen st�mmer, det ser extremt starkt ut i rendering[27i12~10]
...
//simple geometry, determines distance between screen and origin of the rays
  ...
  for(int x=0; x<size; x+=rectw)
    {
      for(int z=0; z<size; z+=rectw)
	{
	  scan_ray = (Ray){{0}};
	  ...
	  //printf(" <%1.2f %1.2f %1.2f>\n", color.r, color.g, color.b);
	   ...
	   if( silent==0 )
	    drawpixel(rectw, x, z, 256*tmpcol.r, 256*tmpcol.g, 256*tmpcol.b);
	  else
	  {
...
int main(void)
{
  ...
  //if silent we would want to save to file:
